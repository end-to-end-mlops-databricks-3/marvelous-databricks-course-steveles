...
basic_model_b.paramaters = {"learning_rate": 0.01,
                            "n_estimators": 1000,
                            "max_depth": 6}
basic_model_b.model_name = f"{catalog_name}.{schema_name}.rdw_model_basic_B"
...
class HousePriceModelWrapper(mlflow.pyfunc.PythonModel):
    ...
    def predict(self, context, model_input):
        car_id = str(model_input["kenteken"].values[0])
        hashed_id = hashlib.md5(car_id.encode(encoding="UTF-8")).hexdigest()
...
# COMMAND ----------
mlflow.set_experiment(experiment_name="/Shared/rdw-ab-testing")
model_name = f"{catalog_name}.{schema_name}.rdw_model_pyfunc_ab_test"
...
with mlflow.start_run() as run:
    ...
    mlflow.pyfunc.log_model(
        ...
        artifact_path="pyfunc-rdw-price-model-ab",
        artifacts={
...
model_version = mlflow.register_model(
    model_uri=f"runs:/{run_id}/pyfunc-rdw-model-ab", name=model_name, tags=tags.dict()
)
...
def call_endpoint(record):
    ...
    serving_endpoint = f"https://{os.environ['DBR_HOST']}/serving-endpoints/rdw-ab-testing/invocations"
